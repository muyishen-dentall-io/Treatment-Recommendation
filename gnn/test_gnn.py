...
# Edge coordinates (grouped into one trace)
edge_x = []
edge_y = []
edge_opacities = []
...
for i, edge in enumerate(G.edges()):
    ...
    edge_x += [x0, x1, None]
    edge_y += [y0, y1, None]
    edge_opacities.append(0.2 + 0.8 * norm_weights[i])  # scale to [0.2, 1.0]

# Use a single color and vary width or opacity
edge_trace = go.Scatter(
    x=edge_x,
    y=edge_y,
    line=dict(width=1, color='gray'),
    opacity=0.5,  # Average opacity across edges
    mode='lines',
    hoverinfo='none'
)
...
layout = go.Layout(
    title=dict(
        text='Treatment Co-occurrence Graph',
        font=dict(size=18)
    ),
    showlegend=False,
    hovermode='closest',
