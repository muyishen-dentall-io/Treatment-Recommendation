...
class TransformerNextItemPredictor(nn.Module):
    def __init__(self, vocab_size, embed_dim=256, num_heads=4, num_layers=2, dropout=0.1):
        ...
        self.positional_embedding = nn.Embedding(512, embed_dim)  # max seq len = 512
...
class Next_Prediction_Recommender:
    ...
    #     max_len = self.args.MAX_SEQ_LEN  # <- new arg
    #     X, y = [], []
    #     for seq in user_seqs:
    #         for i in range(1, len(seq)):
    #             prefix = seq[:i]
    #             if len(prefix) > max_len:
    #                 prefix = prefix[-max_len:]  # keep last max_len tokens
    #             X.append(prefix)
    #             y.append(seq[i])
    #     return X, y
    ...
    def build_sequences(self, df):
        ...
        decay = self.args.DECAY_ALPHA  # new argument
        X, y = [], []
        for seq in user_seqs:
            for i in range(1, len(seq)):
                prefix = seq[:i]
                ...
                soft_label = {}
                for j, idx in enumerate(seq[i:]):  # next 3 items (next, next-next, etc.)
                    weight = decay ** j
